...
/// Encoding snapshots that pin canonical behavior along two axes:
///
/// - **Multi-script encoding**: a shared Latin/CJK/Cyrillic/Arabic/Thai/emoji input,
///   tokenized against each of the eight models in `TokenizerTests.tokenizer`.
///
...
struct BaselineTests {
    ...
    /// Assert a snapshot: if no expected value is recorded for this model, record the
    /// actual output so it can be pasted back in; otherwise compare.
    private static func expectSnapshot<Value: Equatable>(
        ...
        guard let expected else {
            Issue.record(
                "Baseline missing for \(label). Captured value: \(String(describing: actual))",
