...
The package's PR CI's Linux job filters its test invocation to the offline subset (`AlgorithmTests` + `UniffiSpikeTests`) for this reason. Once the workflow's `swift:6.2.3` container is upgraded to `swift:6.4` or later, the filter can be removed and the full suite should pass on Linux.
